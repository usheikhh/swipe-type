...
//NOTE: This was taken from luiletio's swipetest Github repository https://github.com/luileito/swipetest/blob/master/
...
var keyboardIndex = 0,
  ...
  outFile =
    ...
    "imgs" +
    "/" +
    path.basename(args.logFile, ".log") +
    "-" +
    args.word +
    "-" +
    (+args.failed || 0) +
...
function main(data) {
  ...
  csvContent = csvContent.filter(
    (row) => row.word === args.word && row.isFailedWord === !!args.failed
  );

  if (!csvContent.length) {
    console.error(
      'Word "%s" (failed: %s) not found in %s',
      args.word,
      !!args.failed,
      args.logFile
    );
  ...
  if (numWords > 1) {
    console.warn(
      'Multiple swiping attempts (%d) of word "%s" detected. Will plot the last attempt only.',
      numWords,
      args.word
    );
  }
...
function updateMeasures() {
  ...
  keyboard.settings.keyFontSize = Math.max(
    keyboard.settings.keyFontSize,
    Math.round(keyboard.settings.keyWidth / 2.5)
  );
}
...
function drawKey(k, ctx) {
  ...
  if (keyboard.settings.keyCornerRadius) {
    ctx.roundRect(
      ...
      k.width,
      k.height,
      keyboard.settings.keyCornerRadius
    );
  } else {
  ...
  //   radius =
  //     (1.5 * Math.max(keyboard.settings.keyWidth, keyboard.settings.keyHeight)) /
  //     2;
  //   ctx.arc(center.x, center.y, radius, 0, 2 * Math.PI);
  //   ctx.stroke();
